...
@Injectable()
export class UserAdapter extends EntityAdapter {
    ...
    switch (action.id) {
      ...
      case 'create-entity': { // ! override
        super.invokeAction({
          ...params,
          dialogSettings: {
            newId: `${Math.random().toString(36).slice(2, 10)}${Timestamp.now().toMillis().toString().split('').map((x, idx) => Math.random() < 0.5 ? String.fromCharCode(97 + parseInt(x)).toUpperCase() : String.fromCharCode(97 + parseInt(x))).join('')}${Math.random().toString(36).slice(2, 10)}`,
